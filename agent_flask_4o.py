...
from config import OPENAI_API_FREE_KEY, OPENAI_API_FREE_BASE, HOST, NAME, USER, PASSWORD, SYSTEM_TEMPLATE, EXAMPLES, \
    PREFIX
...
"""
os.environ["OPENAI_API_KEY"] = OPENAI_API_FREE_KEY
os.environ["OPENAI_API_BASE"] = OPENAI_API_FREE_BASE
user = USER
password = PASSWORD
host = HOST
name = NAME
...
# llm输出的sql会带有markdown格式如'```sql\nSELECT * FROM table;\n```'
# 这里使用正则表达式移除Markdown代码块标记
def remove_markdown_format(sql):
...
chain = (
        ...
        .assign(query=lambda x: remove_markdown_format(x['query']))  # 清理Markdown格式
